...
A tool for formatting, validating, minifying, and querying JSON data. Provides real-time validation, syntax highlighting, and useful JSON manipulation features.
...
1. As a developer, I want to format/beautify minified JSON for readability
2. As a developer, I want to minify JSON for production use
3. As a developer, I want to validate JSON and see errors with line numbers
4. As a developer, I want to query JSON using JSONPath expressions
5. As a developer, I want to sort JSON object keys
...
|---------|-------------|----------|
| Format/Beautify | Pretty print with indentation | P0 |
| Minify | Compress to single line | P0 |
| Validate | Check JSON syntax with errors | P0 |
| Copy Result | Copy formatted/minified output | P0 |
| Indent Size | 2, 4, or tab indentation | P1 |
| Sort Keys | Alphabetically sort object keys | P1 |

### Advanced Features

| Feature | Description | Priority |
|---------|-------------|----------|
| JSONPath Query | Extract data using JSONPath | P1 |
| JSON to JS Object | Convert to JavaScript object literal | P2 |
| Flatten/Unflatten | Flatten nested objects | P2 |
| Diff Compare | Compare two JSON documents | P2 |

## Technical Implementation

### Dependencies

**Core**: None (native `JSON.parse`/`stringify`)

**JSONPath**: Optional - `jsonpath-plus`
...
```svelte
<script lang="ts">
  import { FileJson, Copy, Check, RefreshCw, Minimize2, Maximize2, SortAsc, Search, AlertCircle, CheckCircle } from "@lucide/svelte";
  ...
  let jsonPathResult = $state<string | null>(null);
  ...
  // Parse and validate
  function parseJson(input: string): { data: any; error: string | null } {
    try {
  ...
  async function executeJsonPath() {
    ...
    try {
      ...
      const { data } = parseJson(inputJson);
      const result = JSONPath({ path: jsonPathQuery, json: data });
      jsonPathResult = JSON.stringify(result, null, 2);
    } catch (e) {
      jsonPathResult = `Error: ${e instanceof Error ? e.message : "Invalid JSONPath"}`;
    }
  }
  ...
  // Execute JSONPath when query changes
  $effect(() => {
    if (jsonPathQuery) {
      executeJsonPath();
    } else {
...
<div class="container mx-auto p-4 max-w-6xl">
  ...
  {#if isValid}
    <div class="card p-6 bg-surface-50 dark:bg-surface-900 mt-6 space-y-4">
      <h2 class="h2 font-bold flex items-center gap-2">
        <Search class="size-6 text-primary-500" />
        JSONPath Query
      </h2>
      ...
      <div class="flex gap-2">
        <input
          ...
          placeholder="e.g., $.tools[*].name or $..category"
        />
      </div>
      ...
      <div class="text-sm text-surface-500 space-y-1">
        ...
        <ul class="list-disc list-inside space-y-1">
          <li><code class="bg-surface-100 dark:bg-surface-800 px-1 rounded">$.tools</code> - Get tools array</li>
          <li><code class="bg-surface-100 dark:bg-surface-800 px-1 rounded">$.tools[*].name</code> - Get all tool names</li>
          <li><code class="bg-surface-100 dark:bg-surface-800 px-1 rounded">$..features</code> - Find features at any depth</li>
          <li><code class="bg-surface-100 dark:bg-surface-800 px-1 rounded">$.tools[?(@.category=='network')]</code> - Filter by category</li>
        </ul>
      </div>

      {#if jsonPathResult}
        <div class="p-4 bg-surface-100 dark:bg-surface-800 rounded-lg">
          <pre class="font-mono text-sm overflow-auto">{jsonPathResult}</pre>
        </div>
      {/if}
    </div>
  {/if}
  ...
  <!-- Keyboard Shortcuts -->
  <div class="text-sm text-surface-500 mt-6">
    <strong>Tips:</strong> JSON is validated in real-time. Use JSONPath to query specific values.
  </div>
</div>
```
...
```typescript
{
  ...
  desc: "Format, validate & query JSON",
  icon: FileJson,
  href: "/tools/json",
  cat: "encoding",
}
...
- [ ] Format with 2 spaces works
- [ ] Format with 4 spaces works
- [ ] Format with tabs works
- [ ] Minify works correctly
- [ ] Validation detects errors
- [ ] Error line number correct
- [ ] Sort keys works
...
**3-4 hours** - Native JSON API handles most functionality.

## Notes

- Native `JSON.parse()` provides good error messages
- JSONPath is optional but adds significant value
- Consider adding JSON Schema validation in future
- Could add flatten/unflatten utilities
