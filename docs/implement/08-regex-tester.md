...
A tool for building and testing regular expressions with real-time matching, capture group visualization, and common pattern presets.
...
1. As a developer, I want to test regex patterns against sample text
2. As a developer, I want to see matched text highlighted in real-time
3. As a developer, I want to view capture groups and their values
4. As a developer, I want to understand complex regex patterns with explanations
...
|---------|-------------|----------|
| Pattern Input | Enter regex pattern | P0 |
| Test Text | Input text to test against | P0 |
| Real-time Matching | Live highlight of matches | P0 |
| Match Count | Display number of matches | P0 |
| Capture Groups | Show captured groups | P0 |
| Flags Toggle | g, i, m, s, u flags | P0 |
| Copy Regex | Copy pattern for code | P1 |
...
| Date | Match date formats |
| Credit Card | Match card numbers |
| Hex Color | Match #RRGGBB |

### Output Features

| Feature | Description |
|---------|-------------|
| Matched Text | Highlighted in original text |
| Match List | List of all matches |
| Group Values | Each capture group value |
| Match Indices | Start/end positions |
| Explanation | Plain English pattern breakdown |
...
```svelte
<script lang="ts">
  import { Regex, Copy, Check, HelpCircle, BookOpen, AlertCircle, CheckCircle } from "@lucide/svelte";
  ...
  function getHighlightedText(): string {
    ...
    // Sort matches by index descending for replacement
    const sorted = [...matches].sort((a, b) => b.index - a.index);
    let result = testText;

    for (const match of sorted) {
      const before = result.slice(0, match.index);
      const after = result.slice(match.index + match.value.length);
      result = `${before}[[MATCH]]${match.value}[[/MATCH]]${after}`;
    }
...
<div class="container mx-auto p-4 max-w-6xl">
  ...
  <div class="card p-6 bg-surface-50 dark:bg-surface-900 space-y-4 mb-6">
    <div class="flex justify-between items-center">
      <label class="font-medium">Regular Expression</label>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    ...
    <div class="card p-4 bg-surface-50 dark:bg-surface-900 space-y-2">
      <label class="font-medium">Test Text</label>
      <textarea
    ...
    <div class="card p-4 bg-surface-50 dark:bg-surface-900 space-y-4">
      <div class="flex justify-between items-center">
        <label class="font-medium">Matches</label>
        {#if matches.length > 0}
...
- [ ] Capture groups displayed
- [ ] Named groups displayed
- [ ] Presets apply correctly
- [ ] Code snippets generated
- [ ] Copy to clipboard works
- [ ] Error messages shown
- [ ] Explanation breakdown helpful
- [ ] Help reference complete
- [ ] Dark mode styling
- [ ] Mobile responsive

## Estimated Effort

**4-5 hours** - Native RegExp handles matching, UI complexity in highlighting.

## Notes

- Native `RegExp.exec()` in loop for global matches
- Consider adding regex-to-string explanation library for better breakdowns
- Could add "generate regex from examples" feature
- Regex explanation is basic; could integrate with a library for more detail
