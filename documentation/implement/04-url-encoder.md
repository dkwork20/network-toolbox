...
A tool for encoding and decoding URL components. Handles URL encoding (percent-encoding), URL parsing, and query string manipulation.
...
1. As a developer, I want to encode query parameters for API calls
2. As a developer, I want to decode URL-encoded strings to inspect them
3. As a developer, I want to parse a full URL into its components
4. As a developer, I want to build query strings from key-value pairs
...
|---------|-------------|----------|
| URL Encoding | Encode text for use in URLs | P0 |
| URL Decoding | Decode URL-encoded text | P0 |
| Full URL Parsing | Break down URL into components | P0 |
| Query String Builder | Build query strings from key-value pairs | P1 |
| Batch Mode | Encode/decode multiple lines | P1 |

### URL Components Display

When parsing a full URL, display:
- Protocol (http, https, ws, wss, ftp)
- Hostname / Domain
- Port
- Pathname
...
// url.protocol, url.hostname, url.port, url.pathname, url.search, url.hash
```

### File Structure

```
src/routes/tools/url/
└── +page.svelte
```
...
- [ ] URL parsing extracts all components
- [ ] Query parameters displayed correctly
- [ ] Query string builder works
- [ ] Add/remove query params works
...
**3-4 hours** - Native APIs handle most functionality.

## Notes

- `encodeURIComponent()` vs `encodeURI()`:
  - `encodeURIComponent()` encodes all special chars (for query values)
  - `encodeURI()` preserves URL structure chars (for full URLs)
- URL constructor requires a base URL for relative URLs
- Consider adding `encodeURI()` option for full URL encoding
