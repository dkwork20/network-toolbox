...
A tool for generating secure, random passwords with customizable options including length, character sets, and passphrase mode.
...
1. As a user, I want to generate strong passwords for my accounts
2. As a user, I want to customize password complexity requirements
3. As a user, I want to generate memorable passphrases
...
|---------|-------------|----------|
| Random Password | Generate secure random password | P0 |
| Customizable Length | 4-128 characters | P0 |
| Character Sets | Toggle uppercase, lowercase, digits, symbols | P0 |
| Copy to Clipboard | One-click copy | P0 |
| Strength Indicator | Visual password strength meter | P0 |
| Bulk Generation | Generate multiple passwords | P1 |
| Passphrase Mode | Generate word-based passwords | P1 |
| Exclude Ambiguous | Remove similar chars (0O, l1, etc.) | P1 |
| Pronounceable | Generate pronounceable passwords | P2 |

### Character Sets

| Set | Characters | Default |
|-----|------------|---------|
| Uppercase | A-Z | On |
| Lowercase | a-z | On |
| Digits | 0-9 | On |
| Symbols | !@#$%^&*()_+-=[]{}|;:,.<>? | Off |
| Ambiguous | 0O1lI | Excluded by option |

### Password Strength Levels

| Level | Criteria | Visual |
|-------|----------|--------|
| Very Weak | < 8 chars | Red |
| Weak | 8+ chars, limited chars | Orange |
| Fair | 12+ chars, mixed | Yellow |
| Strong | 16+ chars, all types | Green |
| Very Strong | 20+ chars, all types, high entropy | Blue |

## Technical Implementation

### Dependencies

**None** - Uses native `crypto.getRandomValues()`

```typescript
// Secure random number generation
const array = new Uint32Array(1);
crypto.getRandomValues(array);
const randomIndex = array[0] % max;
```

### File Structure

```
src/routes/tools/password/
└── +page.svelte
```

### Word List for Passphrases

Consider embedding a word list (e.g., EFF's large wordlist ~7776 words) or use a smaller built-in list:
...
```svelte
<script lang="ts">
  import { KeyRound, Copy, Check, RefreshCw, Shield, ShieldAlert, ShieldCheck, ShieldQuestion, ShieldX, Download, List } from "@lucide/svelte";
  ...
  // Word list for passphrases (simplified - in production, use full EFF list)
  const wordList = [
    "abandon", "ability", "able", "about", "above", "absent", "absorb", "abstract",
  ...
  function getStrengthIcon(level: StrengthLevel) {
    switch (level) {
      ...
      case "fair": return ShieldQuestion;
      case "strong": return ShieldCheck;
      case "very-strong": return Shield;
    }
  }
    ...
    {#if password}
      <div class="p-4 bg-surface-100 dark:bg-surface-800 rounded-lg">
        ...
        <div class="mt-4 flex items-center gap-3">
          <svelte:component this={getStrengthIcon(strength.level)} class="size-5 {getStrengthColor(strength.level)}" />
          <div class="flex-1">
            <div class="h-2 bg-surface-300 dark:bg-surface-600 rounded-full overflow-hidden">
              <div
                class="h-full transition-all {getStrengthColor(strength.level).replace('text-', 'bg-')}"
                style="width: {(strength.score / 8) * 100}%"
              ></div>
            </div>
          </div>
          <span class="text-sm font-medium {getStrengthColor(strength.level)} capitalize">
            {strength.level.replace('-', ' ')}
          </span>
        </div>
        <p class="text-xs text-surface-500 mt-2">
          Entropy: {strength.entropy.toFixed(1)} bits | Length: {password.length}
        </p>
      </div>
    {/if}
        ...
        <div class="max-h-60 overflow-y-auto space-y-2">
          {#each passwords as pwd, index}
            <div class="flex justify-between items-center p-2 bg-surface-100 dark:bg-surface-800 rounded">
              <code class="font-mono text-sm">{pwd}</code>
              <button class="btn-icon btn-icon-sm" onclick={() => copyPassword(index)}>
...
- [ ] All character sets work correctly
- [ ] Length slider adjusts password length
- [ ] Exclude ambiguous characters works
- [ ] Passphrase mode generates word-based passwords
- [ ] Separator options work
...
**4-6 hours** - Native crypto API, word list embedded.

## Notes

- Uses `crypto.getRandomValues()` for cryptographically secure random numbers
- Word list is EFF's large wordlist (simplified version shown, use full list for production)
- Consider adding password history (localStorage) for "recently generated"
- Could add password strength analyzer for pasted passwords
